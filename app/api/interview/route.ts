...
import { codingGaps, type CodingGap } from "@/lib/interview";
import { parseQuestions, parseFeedback } from "@/lib/validation";
  ...
  try {
    ...
    if (input.action === "questions") {
      ...
      try {
        ...
        const questions = parseQuestions(result.data?.questions);
        if (questions) {
    ...
    if (input.action === "feedback") {
      ...
      const codingTopic = input.phase === "technical" && codingGaps.includes(input.codingTopic) ? input.codingTopic as CodingGap : undefined;
      try {
        ...
        const feedback = parseFeedback(result.data, codingTopic);
        if (feedback) {
